...
function RoutePieChart({ routes, colorMap}: RoutePieChartProps): ReactElement {
    ...
    r.features.forEach((f: GeoJSON.Feature) => {
      ...
      (f.geometry as GeoJSON.LineString).coordinates.forEach(coord => {
        if (f.properties) {
          f.properties["lengthInMeters"] += haversine({ lat: last[1], lon: last[0] }, { lat: coord[1], lon: coord[0] })
        }
        last = coord
      })
    })
  ...
  const byWayId = routes.flatMap(r => r.features).reduce((acc: Map<string | number, GeoJSON.Feature>, curr) => {
  ...
  const lts1Length = groups[1]?.reduce((acc, curr) => acc + curr.properties!["lengthInMeters"], 0) ?? 0
  const lts2Length = groups[2]?.reduce((acc, curr) => acc + curr.properties!["lengthInMeters"], 0) ?? 0
  const lts3Length = groups[3]?.reduce((acc, curr) => acc + curr.properties!["lengthInMeters"], 0) ?? 0
  const lts4Length = groups[4]?.reduce((acc, curr) => acc + curr.properties!["lengthInMeters"], 0) ?? 0
  ...
  return (
     <div className='pie-chart'>
              <PieChart data={pieChartData} animate={true} label={({ dataEntry }) => Math.round(dataEntry.percentage) + '%'}
                labelPosition={100 - lineWidth / 2} lineWidth={lineWidth}
                labelStyle={{
                  fill: '#fff',
                  opacity: 0.75,
                  pointerEvents: 'none',
