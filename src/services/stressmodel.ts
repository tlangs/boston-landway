...
type LTSResult = { lts: number, message: [] };

var exports = {

  name: 'default',
  description: 'Standard daylight model. This model looks at roads as a single unit and does not take travel direction or intersection approaches into account.',
  version: '1.0.1',
  levels: 4,
  tags: ['access', 'bicycle', 'construction', 'cycleway', 'footway', 'highway', 'lanes', 'maxspeed', 'motor_vehicle', 'parking', 'service', 'shoulder'],
  usesTag: function (tag: string) {
...
export function evaluateLTS(way: OverpassWay) {
  const bikingPermittedAnalyzed = bikingPermitted(way);
  if (!bikingPermittedAnalyzed.permitted) {
    return bikingPermittedAnalyzed.result;
  }
  const separatedPathAnalyzed = isSeparatedPath(way);
  if (separatedPathAnalyzed.isSeparatedPath) {
    return separatedPathAnalyzed.result;
  }
  const bikeLaneAnalyzed = isBikeLane(way);
  if (bikeLaneAnalyzed.isBikeLane) {
    return bikeLaneAnalyzed.result;
  }
  const mixedTrafficAnalyzed = isMixedTraffic(way);
  if (mixedTrafficAnalyzed.isMixedTraffic) {
    return mixedTrafficAnalyzed.result;
  }
  return { lts: 0, message: ['Error: This way does not match any of the analysis criteria.'] }
}

export function isBikingPermitted(way: OverpassWay) {
  return bikingPermitted(way).permitted
}

function HasTag(way: OverpassWay, tag: string) {
  return (typeof way?.tags?.[tag] !== 'undefined')
}

function HasTagValue(way: OverpassWay, tag: string, tagvalue: string) {
  let x = way?.tags?.[tag]
  if (typeof x === 'undefined') return false
  return x === tagvalue
}

function TagStartsWith(way: OverpassWay, tag: string) {
...
function TagStartsWithValue(way: OverpassWay, tag: string, tagvalue: any) {
  for (let t in way.tags) {
    if (t.startsWith(tag)) {
...
function getLanes(way: OverpassWay): { lanes: number, message: string[]} {
  let lanes
  let defaultLanes = 2
  if (way?.tags?.['lanes']) {
    const lanesTagValue = way.tags.lanes;
    if (lanesTagValue.indexOf(';') === -1) {
      lanes = parseInt(lanesTagValue)
      if (isNaN(lanes)) {
        return { 'lanes': defaultLanes, message: [`Error: Unknown 'lanes' tag: '${lanesTagValue}'. Assuming default of '${defaultLanes}'.`] }
      } else {
        return { 'lanes': lanes, message: [] }
      }
    } else {
      let list = lanesTagValue.split(';')
      let ltot = 0
      for (let s in list) {
        lanes = parseInt(s)
        if (isNaN(lanes)) {
          return { 'lanes': defaultLanes, message: [`Error: Unknown 'lanes' tag in split value: '${lanesTagValue}'. Assuming default of '${defaultLanes}'.`] }
        } else {
          ltot += lanes
        }
      }
    ...
    return { 'lanes': defaultLanes, message: [`No 'lanes' tag. Assuming default of '${defaultLanes}.`] }
  }
}

function getMaxSpeed(way: OverpassWay): { maxspeed: number, message: string[]} {
  if (way?.tags?.['maxspeed']) {
    const maxspeed = way.tags.maxspeed
    const speedInMph = maxspeed.includes("mph") ? parseInt(maxspeed.split(" ")[0]) : Math.round(parseInt(maxspeed) * 0.62137119);
    if (isNaN(speedInMph)) {
      return { maxspeed: 50, message: [`Error: Unknown 'maxspeed' tag value '${maxspeed}'. Assuming a value of '50 mph'.`] }
    }

    else {
      return { maxspeed: speedInMph, message: [] }
    }
  } else {
    const highwayTagValue = way?.tags?.['highway'];
    switch (highwayTagValue) {
      case 'motorway': return { maxspeed: 65, message: ['No maxspeed tag. Using default value of 65 mph when highway=motorway.'] };
      case 'trunk': return { maxspeed: 45, message: ['No maxspeed tag. Using default value of 45 mph when highway=trunk.'] };
      case 'primary': return { maxspeed: 30, message: ['No maxspeed tag. Using default value of 30 mph when highway=primary.'] };
      case 'secondary':
      case 'residential': return { maxspeed: 25, message: [`No maxspeed tag. Using default value of 25 mph when highway=${highwayTagValue}.`] };
      default: return { maxspeed: 50, message: [`No maxspeed tag. Using default value of 50 mph when highway='${highwayTagValue}'.`] }
    }
  }
}

function parkingPresent(way: OverpassWay) {
...
function hasSeparatingMedian(way: OverpassWay) {
  return false
}

function bikeAndParkingWidth(way: OverpassWay) {
...
function bikingPermitted(way: OverpassWay) {
...
function isSeparatedPath(way: OverpassWay) {
  let rule = ''
  let message = []
...
function bikeLaneAnalysisParkingPresent(way: OverpassWay, message: string[]) {
  let rule = ''
  ...
  } else if (width < 4.5 && (maxspeed < 25 || isResidential)) {
    if (lts < 2) {
      rule = 'b5'
      message.push('Increasing LTS to 2 because the bike lane width is less than 4.5m. maxspeed is less than 25 mph on a residential street and parking present.')
      lts = 2
    }
  }
  ...
  if (maxspeed > 25) {
    if (maxspeed <= 30) {
      if (lts < 2) {
        rule = 'b6'
        message.push('Increasing LTS to 2 because the maxspeed is between 25-30 mph and parking present.')
        lts = 2
      }
    } else if (maxspeed <= 35) {
      if (lts < 3) {
        rule = 'b7'
        message.push('Increasing LTS to 3 because the maxspeed is between 30-35 km/h and parking present.')
        lts = 3
      }
    } else {
      if (lts < 4) {
        rule = 'b8'
        message.push('Increasing LTS to 4 because the maxspeed is over 35 mph and parking present.')
        lts = 4
      }
    }
  }
  ...
  if (lts === 1) {
    rule = 'b1'
    message.push('LTS is 1 because there is parking present, the maxspeed is less than or equal to 25mph, highway=\'residential\', and there are 2 lanes or less.')
  }

  return { lts: lts, message: message, rule: rule }
}

function bikeLaneAnalysisNoParking(way: OverpassWay, message: string[]) {
  let rule = ''
  const isResidential = HasTagValue(way, 'highway', 'residential')
  const width = bikeAndParkingWidth(way)
  ...
  if (maxspeed > 30) {
    if (maxspeed <= 40) {
      if (lts < 3) {
        rule = 'c5'
        message.push('Increasing LTS to 3 because the maxspeed is between 30-40 mph and no parking.')
        lts = 3
      }
    } else {
      if (lts < 4) {
        rule = 'c6'
        message.push('Increasing LTS to 4 because the maxspeed is over 40 mph and no parking.')
        lts = 4
      }
    }
  }
  ...
  if (lts === 1) {
    rule = 'c1'
    message.push('LTS is 1 because there is no parking, maxspeed is less than or equal to 30 mph, highway=\'residential\', and there are 2 lanes or less.')
  }

  return { lts: lts, message: message, rule: rule }
}

function isBikeLane(way: OverpassWay) {
  let result
  let analyze = false
  let message: string[] = []
...
function isMixedTraffic(way: OverpassWay) {
  ...
  if (maxspeed <= 30) {
    if (HasTagValue(way, 'highway', 'service')) {
      if (HasTagValue(way, 'service', 'parking_aisle')) {
        message.push('Setting LTS to 2 because maxspeed is 30 mph or less and service is \'parking_aisle\'.')
        return { isMixedTraffic: true, result: { lts: 2, message: message, rule: 'm3' } }
      }
      if (HasTagValue(way, 'service', 'driveway')) {
        message.push('Setting LTS to 2 because maxspeed is 30 mph or less and service is \'driveway\'.')
        return { isMixedTraffic: true, result: { lts: 2, message: message, rule: 'm4' } }
      }
      if (maxspeed <= 25) {
        message.push('Setting LTS to 2 because maxspeed is less than 25 mph and highway=\'service\'.')
        return { isMixedTraffic: true, result: { lts: 2, message: message, rule: 'm16' } }
      }
    }
    if (maxspeed <= 25) {
      if (lanes <= 3 && isResidential) {
        message.push('Setting LTS to 2 because maxspeed is up to 25 mph, 3 or fewer lanes and highway=\'residential\'.')
        return { isMixedTraffic: true, result: { lts: 2, message: message, rule: 'm5' } }
      } else if (lanes <= 3) {
        message.push('Setting LTS to 3 because maxspeed is up to 25 mph and 3 or fewer lanes on non-residential highway.')
        return { isMixedTraffic: true, result: { lts: 3, message: message, rule: 'm6' } }
      } else if (lanes <= 5) {
        message.push('Setting LTS to 3 because maxspeed is up to 25 mph and 4 or 5 lanes.')
        return { isMixedTraffic: true, result: { lts: 3, message: message, rule: 'm7' } }
      } else {
        message.push('Setting LTS to 4 because maxspeed is up to 25 mph and the number of lanes is greater than 5.')
        return { isMixedTraffic: true, result: { lts: 4, message: message, rule: 'm8' } }
      }
    } else {
      if (lanes < 3 && isResidential) {
        message.push('Setting LTS to 2 because maxspeed is up to 30 mph and lanes are 2 or less and highway=\'residential\'.')
        return { isMixedTraffic: true, result: { lts: 2, message: message, rule: 'm9' } }
      } else if (lanes <= 3) {
        message.push('Setting LTS to 3 because maxspeed is up to 30 mph and lanes are 3 or less on non-residential highway.')
    ...
    message.push('Setting LTS to 4 because maxspeed is greater than 30 mph.')
